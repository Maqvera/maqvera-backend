...
- **Queue-Based Processing, Parallel Workers, Queue Redundancy, Dead Letter Queue (as separate infra)** — no message-broker/job-queue exists (`utils/eventBus.js` itself documents `EVENT_BUS_TRANSPORT=memory` as in-process-only); the Payment Retry Engine's own bounded manual-retry count is the honest in-application equivalent.
